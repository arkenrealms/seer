...
- Revalidated protocol Rush lock metadata (`packages/protocol/.rush/temp/shrinkwrap-deps.json`) and refreshed generated-lock ownership notes.
...
- Refactored `packages/node/src/tests.ts` guard checks to use a shared `requireMethod` helper for consistent failure semantics across helper entrypoints.
